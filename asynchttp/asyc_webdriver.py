...
# @Date : 2022/3/5
# @Author : Ph
# @File : asyc_webdriver.py

from asynchttp.asyc_http_client import Command
from aiohttp import ClientSession
from asynchttp.asyc_element import Element
...
class AsyncBrowser(Command):
    ...
    async def find_element(self, by: str, value: str, endpoint: str = '/element'):
        if by == 'id':
            by = 'css selector'
            value = '[id="%s"]' % value
        elif by == 'name':
        ...
        return Element(element=element, url=self._url, session=self._session)
